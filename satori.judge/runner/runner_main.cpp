...
static struct
{
  ...
  const char*   stdout;
  int           stdouttrunc;
  long long     stdoutsize;
  const char*   stderr;
  int           stderrtrunc;
  long long     stderrsize;
  const char*   setuid;
  const char*   setgid;
  int           ptrace;
  int           memlock;
...
static struct poptOption CONFIG_OPT[] =
{
  ...
  { "stdout-size",     0,   POPT_ARG_LONGLONG, &config.stdoutsize,     0, "standard output maximum size",                       "LIMIT" },
  { "stderr-size",     0,   POPT_ARG_LONGLONG, &config.stderrsize,     0, "standard error maximum size",                        "LIMIT" },
  ...
  { "max-memory",      0,   POPT_ARG_LONGLONG, &config.memory,         0, "limit memory usage to LIMIT bytes",                  "LIMIT" },
  { "max-stack",       0,   POPT_ARG_LONGLONG, &config.stack,          0, "limit stack usage to LIMIT bytes",                   "LIMIT" },
  { "max-data",        0,   POPT_ARG_LONGLONG, &config.data,           0, "limit data usage to LIMIT bytes",                    "LIMIT" },
  { "max-files",       0,   POPT_ARG_LONG,     &config.files,          0, "limit open file count to LIMIT",                     "LIMIT" },
  { "max-read",        0,   POPT_ARG_LONGLONG, &config.io_read,        0, "limit write to LIMIT bytes",                         "LIMIT" },
  { "max-write",       0,   POPT_ARG_LONGLONG, &config.io_write,       0, "limit read to LIMIT bytes",                          "LIMIT" },
  ...
  { "cgroup-memory",   0,   POPT_ARG_LONGLONG, &config.c_memory,       0, "limit memory usage to LIMIT bytes for control group",      "LIMIT" },
...
int main(int argc, const char** argv)
{
  ...
  if (config.stdoutsize > 0)
      run.output_size = config.stdoutsize;
  if (config.stderrsize > 0)
      run.error_size = config.stderrsize;
  if (config.threads > 0)
