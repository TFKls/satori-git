...
#    print 'submits:  ', submits
    ...
    for name, problem in sorted(problems.iteritems()):
        ...
        Creator('Ranking', contest=contest, name='Ranking').fields(is_public=True, aggregator='ACMAggregator').additional(params=params.get_map(), problem_test_suites={}, problem_test_suite_params={})()
        params = OaMap()
        params.set_str('time_start', time_start.strftime('%Y-%m-%d %H:%M:%S'))
        params.set_str('hide_invisible', '0')
        Creator('Ranking', contest=contest, name='Full Ranking').fields(is_public=False, aggregator='ACMAggregator').additional(params=params.get_map(), problem_test_suites={}, problem_test_suite_params={})()
