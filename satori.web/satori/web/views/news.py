...
def add(request, page_info):
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            message = Subpage.create_global(SubpageStruct(is_announcement=True,contest=page_info.contest,name=data["name"],content=data["content"], is_sticky=data["is_sticky"], is_public=data["is_public"]))
...
def edit(request, page_info,id):
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            data = form.cleaned_data
            message.modify(SubpageStruct(name=data["name"],content=data["content"], is_sticky=data["is_sticky"], is_public=data["is_public"]))
            return HttpResponseRedirect(reverse('news'))
    else:
        form = NewsEditForm(initial={'name' : message.name, 'content' : message.content, 'is_public' : message.is_public, 'is_sticky' : message.is_sticky})
